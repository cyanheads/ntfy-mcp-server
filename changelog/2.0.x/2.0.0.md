---
summary: "Major rewrite on @cyanheads/mcp-ts-core. New 4-tool surface (publish, manage, fetch, emoji search), two ntfy resources, env-driven config, retry-aware HTTP client. Breaking changes from 1.x."
breaking: true
security: false
---
...
- **`ntfy_publish_message`** tool — single tool covering all 18 publish parameters, including titles, priorities, tags, click/attach/icon URLs, action buttons (`view` / `broadcast` / `http`), email/SMS/phone-call delivery, scheduled delivery, markdown bodies, and message updates via `sequence_id`.
...
- **`ntfy://emojis`** resource — full emoji-tag reference rendered as Markdown for tool-only clients.
...
- **`send_ntfy`** tool — replaced by `ntfy_publish_message`. Existing 1.x callers must rename and re-map parameters (snake_case throughout).
