...
`parseEnvConfig` maps Zod schema paths → env var names so validation errors name the actual variable (`NTFY_AUTH_TOKEN`) rather than the internal path (`authToken`). It throws a `ConfigurationError` the framework catches and prints as a clean startup banner, and reads an empty string or a whole-value `${…}` placeholder as unset. This server's own loader (`loadFromEnv`) assembles the `NTFY_SERVERS` / single-server registry by hand, so it runs the raw env through `normalizeEnv` from `@cyanheads/mcp-ts-core/config` first to get the same treatment, and maps issue paths back to env var names itself.
